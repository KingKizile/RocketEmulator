...
namespace Rocket.HabboHotel.Users.Messenger
{
    public class HabboMessenger
    {
        ...
        public void ProcessOfflineMessages()
        {
            ...
            using (IQueryAdapter dbClient = RocketEmulador.GetDatabaseManager().GetQueryReactor())
            {
                ...
                if (GetMessages != null)
                {
                    GameClient Client = RocketEmulador.GetGame().GetClientManager().GetClientByUserID(this._userId);
                    if (Client == null)
                        return;
        ...
        public void Destroy()
        {
            IEnumerable<GameClient> onlineUsers = RocketEmulador.GetGame().GetClientManager().GetClientsById(_friends.Keys);
        ...
        public void CreateFriendship(int friendID)
        {
            ...
            GameClient User = RocketEmulador.GetGame().GetClientManager().GetClientByUserID(friendID);
            ...
            if (User != null)
                RocketEmulador.GetGame().GetAchievementManager().ProgressAchievement(User, "ACH_FriendListSize", 1);

            GameClient thisUser = RocketEmulador.GetGame().GetClientManager().GetClientByUserID(_userId);
            if (thisUser != null)
                RocketEmulador.GetGame().GetAchievementManager().ProgressAchievement(thisUser, "ACH_FriendListSize", 1);
        }
        ...
        {
            GameClient friend = RocketEmulador.GetGame().GetClientManager().GetClientByUserID(friendID);
            ...
            if (friend == null || friend.GetHabbo() == null)
            {
                ...
                newFriend = new MessengerBuddy(friendID, Convert.ToString(dRow["username"]), Convert.ToString(dRow["look"]), Convert.ToString(dRow["motto"]), Convert.ToInt32(dRow["last_online"]),
                    RocketEmulador.EnumToBool(dRow["hide_online"].ToString()), RocketEmulador.EnumToBool(dRow["hide_inroom"].ToString()));
            }
            else
            {
        ...
        public bool RequestBuddy(string UserQuery)
        {
            ...
            GameClient client = RocketEmulador.GetGame().GetClientManager().GetClientByUsername(UserQuery);
            if (client == null)
            {
                DataRow Row = null;
                using (IQueryAdapter dbClient = RocketEmulador.GetDatabaseManager().GetQueryReactor())
                ...
                hasFQDisabled = RocketEmulador.EnumToBool(Row["block_newfriends"].ToString());
            }
            else
            {
            ...
            RocketEmulador.GetGame().GetQuestManager().ProgressUserQuest(GetClient(), QuestType.ADD_FRIENDS);

            GameClient ToUser = RocketEmulador.GetGame().GetClientManager().GetClientByUserID(ToId);
            if (ToUser == null || ToUser.GetHabbo() == null)
                return true;

            MessengerRequest Request = new MessengerRequest(ToId, _userId, RocketEmulador.GetGame().GetClientManager().GetNameById(_userId));

            ToUser.GetHabbo().GetMessenger().OnNewRequest(_userId);

            UserCache ThisUser = RocketEmulador.GetGame().GetCacheManager().GenerateUser(_userId);
        ...
        public void OnNewRequest(int friendID)
        {
            if (!_requests.ContainsKey(friendID))
                _requests.Add(friendID, new MessengerRequest(_userId, friendID, RocketEmulador.GetGame().GetClientManager().GetNameById(friendID)));
        }
        ...
        public void SendInstantMessage(int ToId, string Message)
        {
            ...
            {
                GetClient().GetHabbo().MessengerSpamTime = RocketEmulador.GetUnixTimestamp() + 60;
                GetClient().GetHabbo().MessengerSpamCount = 0;
                GetClient().SendNotification("Você não pode enviar mensagem, por que está floodando..\n\nAguarde 60 segundos para enviar um nova mensagem..");
                return;
            }
            else if (GetClient().GetHabbo().MessengerSpamTime > RocketEmulador.GetUnixTimestamp())
            {
                double Time = GetClient().GetHabbo().MessengerSpamTime - RocketEmulador.GetUnixTimestamp();
                GetClient().SendNotification("Você não pode enviar mensagem, por que está floodando..\n\nVocê poderá enviar mensagem em " + Time + " segundos.");
                return;
            }
            ...
            GameClient Client = RocketEmulador.GetGame().GetClientManager().GetClientByUserID(ToId);
            if (Client == null || Client.GetHabbo() == null || Client.GetHabbo().GetMessenger() == null)
            {
                using (IQueryAdapter dbClient = RocketEmulador.GetDatabaseManager().GetQueryReactor())
                {
        ...
        {
            IEnumerable<GameClient> MyFriends = RocketEmulador.GetGame().GetClientManager().GetClientsById(this._friends.Keys);
        ...
        {
            return RocketEmulador.GetGame().GetClientManager().GetClientByUserID(this._userId);
        }
