...
namespace Rocket.HabboHotel.Users.UserDataManagement
{
    public class UserDataFactory
    {
        public static UserData GetUserData(string SessionTicket, out byte errorCode)
        {
            ...
            {
                dbClient.SetQuery("SELECT `id`,`username`,`rank`,`motto`,`look`,`gender`,`last_online`,`credits`,`activity_points`,`home_room`,`block_newfriends`,`hide_online`,`hide_inroom`,`vip`,`account_created`,`vip_points`,`machine_id`,`nux_user`,`is_nuevo`,`volume`,`chat_preference`,`focus_preference`, `pets_muted`,`bots_muted`,`advertising_report_blocked`,`last_change`,`gotw_points`,`ignore_invites`,`time_muted`,`allow_gifts`,`friend_bar_state`,`disable_forced_effects`,`allow_mimic`, `allow_events`, `allow_commands`, `rank_vip`  FROM `users` WHERE `auth_ticket` = @sso LIMIT 1");
                ...
                if (RocketEmulador.GetGame().GetClientManager().GetClientByUserID(UserId) != null)
                {
                    errorCode = 2;
                    RocketEmulador.GetGame().GetClientManager().GetClientByUserID(UserId).Disconnect();
                    return null;
                }
            ...
            foreach (DataRow dRow in dFriends.Rows)
            {
                ...
                bool friendHideOnline = RocketEmulador.EnumToBool(dRow["hide_online"].ToString());
                bool friendHideRoom = RocketEmulador.EnumToBool(dRow["hide_inroom"].ToString());
            ...
            {
                rooms.Add(RocketEmulador.GetGame().GetRoomManager().FetchRoomData(Convert.ToInt32(dRow["id"]), dRow));
            }
           ...
           // if (user.Rank >= 7) //Admi
               // friends.Add(1000000005, new MessengerBuddy(1000000005, "Chat da Administrativo", "hr-831-45.fa-1206-91.sh-290-1331.ha-3129-100.hd-180-2.cc-3039-73.ch-3215-92.lg-270-73", "Solo Staffs3", 0, true, false));


            if (user.Rank >= 5) //Chat da Equipe
                friends.Add(1000000004, new MessengerBuddy(1000000004, "Chat da Equipe", "hr-831-45.fa-1206-91.sh-290-1331.ha-3129-100.hd-180-2.cc-3039-73.ch-3215-92.lg-270-73", "Solo Staffs3", 0, true, false));

            if (user.Rank >= 4) //Chat dos divulgadores
                friends.Add(1000000003, new MessengerBuddy(1000000003, "Chat dos Divulgadores", "hr-831-45.fa-1206-91.sh-290-1331.ha-3129-100.hd-180-2.cc-3039-73.ch-3215-92.lg-270-73", "Solo Staffs3", 0, true, false));


            if (user.Rank == 3) //Chat dos locutores
                friends.Add(1000000002, new MessengerBuddy(1000000002, "Chat dos Locutores", "ch-255-62.hd-180-1.ha-3144-62-62.lg-275-92.ca-1813-62.sh-290-93.he-1607-62.hr-830-1035", "Solo Staffs1", 0, true, false));
         

            if (user.Rank == 2) //Chat VIP
                friends.Add(1000000001, new MessengerBuddy(1000000001, "Chat VIP", "ch-255-62.hd-180-1.ha-9027-62.lg-275-92.ca-1813-62.sh-290-93.hr-830-1035", "Solo Staffs", 0, true, false));
        ...
        public static UserData GetUserData(int UserId)
        {
            ...
            using (IQueryAdapter dbClient = RocketEmulador.GetDatabaseManager().GetQueryReactor())
            {
                dbClient.SetQuery("SELECT `id`,`username`,`rank`,`motto`,`look`,`gender`,`last_online`,`credits`,`activity_points`,`home_room`,`block_newfriends`,`hide_online`,`hide_inroom`,`vip`,`account_created`,`vip_points`,`machine_id`,`nux_user`,`is_nuevo`,`volume`,`chat_preference`, `focus_preference`, `pets_muted`,`bots_muted`,`advertising_report_blocked`,`last_change`,`gotw_points`,`ignore_invites`,`time_muted`,`allow_gifts`,`friend_bar_state`,`disable_forced_effects`,`allow_mimic`, `allow_events`, `allow_commands`, `rank_vip` FROM `users` WHERE `id` = @id LIMIT 1");
                dbClient.AddParameter("id", UserId);
                dUserInfo = dbClient.getRow();

                RocketEmulador.GetGame().GetClientManager().LogClonesOut(Convert.ToInt32(UserId));
