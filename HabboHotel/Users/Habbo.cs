...
using Rocket.HabboHotel.Users.Process;
using Rocket.Communication.Packets.Outgoing.Inventory.Purse;


using Rocket.HabboHotel.Users.Navigator.SavedSearches;
...
using Rocket.HabboHotel.Users.Permissions;
using Rocket.HabboHotel.Subscriptions;
using Rocket.HabboHotel.Users.Calendar;
using Rocket.Communication.Packets.Outgoing.Rooms.Notifications;

namespace Rocket.HabboHotel.Users
{
    public enum KickWarsstate { OnRedTeam, OnBlueTeam, Start, End, Exit, None };
    public class Habbo
    {
        private static readonly ILog log = LogManager.GetLogger("Rocket.HabboHotel.Users");

        public KickWarsstate KickWarsState = KickWarsstate.None;
        public bool KickWarsKicked = false;
        ...
        public bool PassedNuxDuckets = false;
        public bool PassedNuxItems = false;
        public bool PassedNuxChat = false;
        public bool PassedNuxCatalog = false;
        ...
        public int dailytrust = 0;
        public bool isOfficer = false;
        public bool twentyone = false;
        ...
        public Habbo(int Id, string Username, int Rank, string Motto, string Look, string Gender, int Credits, int ActivityPoints, int HomeRoom,
           ...
           int GOTWPoints, bool IgnoreInvites, double TimeMuted, double TradingLock, bool AllowGifts, int FriendBarState, bool DisableForcedEffects, bool AllowMimic, bool AllowEvents, bool AllowCommands , int VIPRank)
        {
            this._id = Id;
            this._username = Username;
            this._rank = Rank;
            this._motto = Motto;
            this._look = RocketEmulador.GetGame().GetAntiMutant().RunLook(Look);
            this._gender = Gender.ToLower();
            this._footballLook = RocketEmulador.FilterFigure(Look.ToLower());
            ...
            if (this._tradingLockExpiry > 0 && RocketEmulador.GetUnixTimestamp() > this.TradingLockExpiry)
            {
            ...
            this._sessionStart = RocketEmulador.GetUnixTimestamp();
                ...
                try
                {
                    ...
                    if (Convert.ToString(StatRow["respectsTimestamp"]) != DateTime.Today.ToString("MM/dd"))
                    {
                        ...
                        if (this._permissions.HasRight("mod_tool"))
                            DailyRespects = 3;
                        else if (RocketEmulador.GetGame().GetSubscriptionManager().TryGetSubscriptionData(VIPRank, out SubData))
            ...
            Group G = null;
            if (!RocketEmulador.GetGame().GetGroupManager().TryGetGroup(this._habboStats.FavouriteGroupId, out G))
        ...
        {
            get { return this._allowcommands; }
            set { this._allowcommands = value; }
        }
        ...
        public Room CurrentRoom
        {
            get
            {
                ...
                Room _room = null;
                if (RocketEmulador.GetGame().GetRoomManager().TryGetRoom(CurrentRoomId, out _room))
                    return _room;
        ...
        public void OnDisconnect()
        {
            ...
            RocketEmulador.GetGame().GetClientManager().UnregisterClient(Id, Username);
            ...
            if (!this._habboSaved)
            {
                ...
                using (IQueryAdapter dbClient = RocketEmulador.GetDatabaseManager().GetQueryReactor())
                {
                    dbClient.RunQuery("UPDATE `users` SET `online` = '0', `last_online` = '" + RocketEmulador.GetUnixTimestamp() + "', `activity_points` = '" + this.Duckets + "', `credits` = '" + this.Credits + "', `vip_points` = '" + this.Diamonds + "', `home_room` = '" + this.HomeRoom + "', `gotw_points` = '" + this.GOTWPoints + "', `time_muted` = '" + this.TimeMuted + "',`friend_bar_state` = '" + FriendBarStateUtility.GetInt(this._friendbarState) + "' WHERE id = '" + Id + "' LIMIT 1;UPDATE `user_stats` SET `roomvisits` = '" + this._habboStats.RoomVisits + "', `onlineTime` = '" + (RocketEmulador.GetUnixTimestamp() - this.SessionStart + this._habboStats.OnlineTime) + "', `respect` = '" + this._habboStats.Respect + "', `respectGiven` = '" + this._habboStats.RespectGiven + "', `giftsGiven` = '" + this._habboStats.GiftsGiven + "', `giftsReceived` = '" + this._habboStats.GiftsReceived + "', `dailyRespectPoints` = '" + this._habboStats.DailyRespectPoints + "', `dailyPetRespectPoints` = '" + this._habboStats.DailyPetRespectPoints + "', `AchievementScore` = '" + this._habboStats.AchievementPoints + "', `quest_id` = '" + this._habboStats.QuestID + "', `quest_progress` = '" + this._habboStats.QuestProgress + "', `groupid` = '" + this._habboStats.FavouriteGroupId + "',`forum_posts` = '" + this._habboStats.ForumPosts + "' WHERE `id` = '" + this.Id + "' LIMIT 1;");
        ...
        public void CheckCreditsTimer()
        {
            try
            {
                ...
                {
                    int num = Convert.ToInt32(RocketEmulador.RocketData().data["Moedas"]);
                    int notif = Convert.ToInt32(RocketEmulador.RocketData().data["Duckets"]);
                    int num3 = Convert.ToInt32(RocketEmulador.RocketData().data["Diamantes"]);
                    bool flag2 = Convert.ToBoolean(RocketEmulador.RocketData().data["MensagemAoReceber"]);
                    this._credits += num;
                    this._duckets += notif;
                    this._diamonds += num3;
                    this._client.SendMessage(new CreditBalanceComposer(this._credits));
                    this._client.SendMessage(new HabboActivityPointNotificationComposer(this._duckets, notif, 0));
                    this._client.SendMessage(new HabboActivityPointNotificationComposer(this._diamonds, num3, 5));
                    this.CreditsUpdateTick = Convert.ToInt32(RocketEmulador.RocketData().data["Intervalo"]);
                    if (flag2)
                    {
                       
                        RocketEmulador.GetGame().GetClientManager().GetClientByUsername(this._username).SendMessage(new RoomNotificationComposer("supernoti", "message", "Você recebeu " + num + " moedas " + notif + " duckets e " + num3 + " diamantes"));
                    }
                }
            }
            catch
        ...
        public GameClient GetClient()
        {
            ...
            return RocketEmulador.GetGame().GetClientManager().GetClientByUserID(Id);
        }
        ...
        {
            this.LastNameChange = RocketEmulador.GetUnixTimestamp();
        ...
        public void PrepareRoom(int Id, string Password)
        {
            ...
            {
                Room OldRoom = null;
                if (!RocketEmulador.GetGame().GetRoomManager().TryGetRoom(this.GetClient().GetHabbo().CurrentRoomId, out OldRoom))
                    return;
            ...
            Room Room = RocketEmulador.GetGame().GetRoomManager().LoadRoom(Id);
            if (Room == null)
            {
                this.GetClient().SendMessage(new CloseConnectionComposer());
                return;
        ...
        public bool EnterRoom(Room Room)
        {
            ...
            using (IQueryAdapter dbClient = RocketEmulador.GetDatabaseManager().GetQueryReactor())
            {
                dbClient.RunQuery("INSERT INTO user_roomvisits (user_id,room_id,entry_timestamp,exit_timestamp,hour,minute) VALUES ('" + this.GetClient().GetHabbo().Id + "','" + this.GetClient().GetHabbo().CurrentRoomId + "','" + RocketEmulador.GetUnixTimestamp() + "','0','" + DateTime.Now.Hour + "','" + DateTime.Now.Minute + "');");// +
            }
            ...
            if (Room.OwnerId != this.Id)
            {
                ...
                RocketEmulador.GetGame().GetAchievementManager().ProgressAchievement(this.GetClient(), "ACH_RoomEntry", 1);
            }
            return true;
        }
    }
