...
using Rocket.HabboHotel.Rooms.Instance;
...
using Rocket.HabboHotel.Rooms.Games.Football;
using Rocket.HabboHotel.Rooms.Games.Banzai;
...
namespace Rocket.HabboHotel.Rooms
{
    public class Room : RoomData
    {
        ...
        public List<int> likePollAnswers;
        public List<int> dislikePollAnswers;
        ...
        public void AddBan(int pId, long Time)
        {
            if (!Bans.ContainsKey(Convert.ToInt32(pId)))
                Bans.Add(pId, RocketEmulador.GetUnixTimestamp() + Time);

            using (IQueryAdapter dbClient = RocketEmulador.GetDatabaseManager().GetQueryReactor())
            {
                dbClient.RunQuery("REPLACE INTO `room_bans` VALUES (" + pId + ", " + Id + ", " + (RocketEmulador.GetUnixTimestamp() + Time) + ")");
            }
        }
        ...
        public bool HasBanExpired(int pId)
        {
            ...
            if (Bans[pId] < RocketEmulador.GetUnixTimestamp())
                return true;
        ...
        {
            return this.QuickPoll = new QuickPoll(-2, this, question, duration);
        }
        ...
        public void setPollQuestion(string pool)
        {
            this.pollQuestion = pool;
        }

        public void clearPollAnswers()
        {
            this.likePollAnswers.Clear();
            this.dislikePollAnswers.Clear();
        }
        ...
        public void LoadPromotions()
        {
            ...
            using (IQueryAdapter dbClient = RocketEmulador.GetDatabaseManager().GetQueryReactor())
            {
                ...
                {
                    if (Convert.ToDouble(GetPromotion["timestamp_expire"]) > RocketEmulador.GetUnixTimestamp())
        ...
        public void OnUserShoot(RoomUser User, Item Ball)
        {
            ...
            foreach (Item item in this.GetRoomItemHandler().GetFurniObjects(Ball.GetX, Ball.GetY).ToList())
            {
                if (item.GetBaseItem().ItemName.StartsWith("fball_goal_"))
                {
                    ...
                    RocketEmulador.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_FootballGoalScored", 1);
        ...
        public void ProcessRoom()
        {
            ...
            try
            {
                ...
                {
                    RocketEmulador.GetGame().GetRoomManager().UnloadRoom(this);
                    return;
                }
        ...
        private void OnRoomCrash(Exception e)
        {
            ...
            isCrashed = true;
            RocketEmulador.GetGame().GetRoomManager().UnloadRoom(this, true);
        }
        ...
        public bool CheckMute(GameClient Session)
        {
            ...
            {
                if (MutedUsers[Session.GetHabbo().Id] < RocketEmulador.GetUnixTimestamp())
                {
        ...
        public void AddChatlog(int Id, string Message)
        {
            using (IQueryAdapter dbClient = RocketEmulador.GetDatabaseManager().GetQueryReactor())
            {
                ...
                dbClient.AddParameter("time", RocketEmulador.GetUnixTimestamp());
                dbClient.RunQuery();
            }
        }
