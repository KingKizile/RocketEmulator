...
using Rocket.HabboHotel.Items.Data.Toner;
using Rocket.HabboHotel.Items.Data.Moodlight;
...
namespace Rocket.HabboHotel.Rooms
{
    public class RoomItemHandling
    {
        ...
        internal bool UsedWiredScoreboard;
            ...
            foreach (Item Item in Items.ToList())
            {
                ...
                if (Item.IsFloorItem)
                {
                    if (!_room.GetGameMap().ValidTile(Item.GetX, Item.GetY))
                    {
                        ...
                        GameClient Client = RocketEmulador.GetGame().GetClientManager().GetClientByUserID(Item.UserID);
                        if (Client != null)
                        {
        ...
        public void SaveFurniture()
        {
            try
            {
                bool flag = this._movedItems.Count > 0;
                if (flag)
                {
                    using (IQueryAdapter queryReactor = RocketEmulador.GetDatabaseManager().GetQueryReactor())
                    {
                        foreach (Item current in this._movedItems.Values.ToList<Item>())
                        {
                            bool flag2 = !string.IsNullOrEmpty(current.ExtraData);
                            if (flag2)
                            {
                                ...
                                queryReactor.AddParameter("edata" + current.Id, current.ExtraData);
                                queryReactor.RunQuery();
                            }
                            bool flag3 = current.IsWallItem && (!current.GetBaseItem().ItemName.Contains("wallpaper_single") || !current.GetBaseItem().ItemName.Contains("floor_single") || !current.GetBaseItem().ItemName.Contains("landscape_single"));
                            if (flag3)
                            {
                                queryReactor.SetQuery("UPDATE `items` SET `wall_pos` = @wallPos WHERE `id` = '" + current.Id + "' LIMIT 1");
                                queryReactor.AddParameter("wallPos", current.wallCoord);
                                queryReactor.RunQuery();
                            }
                ...
                {
                    bool flag4 = this.usedwiredscorebord;
                    if (flag4)
                    {
                        this.ScorebordChangeCheck();
                        queryReactor2.RunQuery("DELETE FROM `wired_scorebord`  WHERE roomid = " + this._room.RoomId + " ");
                        Dictionary<int, KeyValuePair<int, string>> wiredScoreBordDay = this._room.WiredScoreBordDay;
                        lock (wiredScoreBordDay)
                        {
                            foreach (int current2 in this._room.WiredScoreBordDay.Keys)
                            {
                                bool flag6 = this._room.WiredScoreBordDay.ContainsKey(current2);
                                if (flag6)
                                {
                                    KeyValuePair<int, string> keyValuePair = this._room.WiredScoreBordDay[current2];
                                    ...
                                    queryReactor2.AddParameter("dusername" + current2, keyValuePair.Value);
                                    queryReactor2.RunQuery();
                                }
                            }
                        }
                        Dictionary<int, KeyValuePair<int, string>> wiredScoreBordMonth = this._room.WiredScoreBordMonth;
                        lock (wiredScoreBordMonth)
                        {
                            foreach (int current3 in this._room.WiredScoreBordMonth.Keys)
                            {
                                bool flag8 = this._room.WiredScoreBordMonth.ContainsKey(current3);
                                if (flag8)
                                {
                                    KeyValuePair<int, string> keyValuePair = this._room.WiredScoreBordMonth[current3];
                                    queryReactor2.SetQuery(string.Concat(new object[]
                                    {
                                        "INSERT INTO `wired_scorebord` (`roomid`, `userid`, `username`, `punten`, `soort`, `timestamp`) VALUES ('",
                                        this._room.RoomId,
                                    ...
                                    queryReactor2.AddParameter("musername" + current3, keyValuePair.Value);
                                    queryReactor2.RunQuery();
                                }
                            }
                        }
                        Dictionary<int, KeyValuePair<int, string>> wiredScoreBordWeek = this._room.WiredScoreBordWeek;
                        lock (wiredScoreBordWeek)
                        {
                            foreach (int current4 in this._room.WiredScoreBordWeek.Keys)
                            {
                                bool flag10 = this._room.WiredScoreBordDay.ContainsKey(current4);
                                if (flag10)
                                {
                                    KeyValuePair<int, string> keyValuePair = this._room.WiredScoreBordDay[current4];
                                    queryReactor2.SetQuery(string.Concat(new object[]
                                    {
                                        "INSERT INTO `wired_scorebord` (`roomid`, `userid`, `username`, `punten`, `soort`, `timestamp`) VALUES ('",
                                        this._room.RoomId,
                                    ...
                                    queryReactor2.AddParameter("wusername" + current4, keyValuePair.Value);
                                    queryReactor2.RunQuery();
                                }
                            }
                        }
        ...
        public bool SetFloorItem(GameClient Session, Item Item, int newX, int newY, int newRot, bool newItem, bool OnRoller, bool sendMessage, bool updateRoomUserStatuses = false)
        {
            ...
            if (newItem)
            {
                ...
                {
                    if (Session != null)
                        Session.SendNotification(RocketEmulador.GetGame().GetLanguageLocale().TryGetValue("room_item_placed"));
                    _room.GetGameMap().RemoveFromMap(Item);
                    return true;
                }
        ...
        public void UpdateWiredScoreBord()
        {
            ...
            foreach (Item current in this._floorItems.Values)
            {
                bool flag = current.GetBaseItem().InteractionType == InteractionType.wired_score_board;
                if (flag)
                {
                    ObjectUpdateComposer item = new ObjectUpdateComposer(current, this._room.OwnerId);
                    list.Add(item);
                }
            }
            this._room.SendMessage(list);
        }


        internal void ScorebordChangeCheck()
        {
            bool flag = this._room.WiredScoreFirstBordInformation.Count == 3;
            if (flag)
            {
                int num = Convert.ToInt32(DateTime.Now.ToString("MMddyyyy"));
                int num2 = Convert.ToInt32(DateTime.Now.ToString("MM"));
                int weekOfYear = CultureInfo.GetCultureInfo("Nl-nl").Calendar.GetWeekOfYear(DateTime.Now, CalendarWeekRule.FirstFourDayWeek, DayOfWeek.Monday);
                List<bool> list = new List<bool>
                {
                    num != this._room.WiredScoreFirstBordInformation[0],
                    num2 != this._room.WiredScoreFirstBordInformation[1],
                    weekOfYear != this._room.WiredScoreFirstBordInformation[2]
                };
                this._room.WiredScoreFirstBordInformation[0] = num;
                this._room.WiredScoreFirstBordInformation[1] = num2;
                this._room.WiredScoreFirstBordInformation[2] = weekOfYear;
                bool flag2 = list[0];
                if (flag2)
                {
                    this._room.WiredScoreBordDay.Clear();
                }
                bool flag3 = list[1];
                if (flag3)
                {
                    this._room.WiredScoreBordMonth.Clear();
                }
                bool flag4 = list[2];
                if (flag4)
                {
                    this._room.WiredScoreBordWeek.Clear();
                }
            }
        }
