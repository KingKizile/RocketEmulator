...
namespace Rocket.HabboHotel.Rooms
{
    public class RoomUserManager
    {
        ...
        public bool AddAvatarToRoom(GameClient Session)
        {
            ...
            // Efeito VIP
            if (Session.GetHabbo().GetPermissions().HasRight("Rocket_vip") && !Session.GetHabbo().DisableForcedEffects)
                Session.GetHabbo().Effects().ApplyEffect(RocketEmulador.RankVIP);
            // Efeito Locutor
            if (Session.GetHabbo().GetPermissions().HasRight("Rocket_loc") && !Session.GetHabbo().DisableForcedEffects)
                Session.GetHabbo().Effects().ApplyEffect(RocketEmulador.RankLOC);
            // Efeito Embaixador
            if (Session.GetHabbo().GetPermissions().HasRight("Rocket_amb") && !Session.GetHabbo().DisableForcedEffects)
                Session.GetHabbo().Effects().ApplyEffect(RocketEmulador.RankAMB);
            // Efeito Programador
            if (Session.GetHabbo().GetPermissions().HasRight("Rocket_pro") && !Session.GetHabbo().DisableForcedEffects)
                Session.GetHabbo().Effects().ApplyEffect(RocketEmulador.RankPRO);
        ...
        public void RemoveUserFromRoom(GameClient Session, Boolean NotifyClient, Boolean NotifyKick = false)
        {
            try
            {
                ...
                if (User != null)
                {
                    ...
                    {
                        dbClient.RunQuery("UPDATE user_roomvisits SET exit_timestamp = '" + RocketEmulador.GetUnixTimestamp() + "' WHERE room_id = '" + _room.RoomId + "' AND user_id = '" + Session.GetHabbo().Id + "' ORDER BY exit_timestamp DESC LIMIT 1");
        ...
        public void OnCycle()
        {
            ...
            try
            {
                ...
                {
                    GameClient client = RocketEmulador.GetGame().GetClientManager().GetClientByUserID(toRemove.HabboId);
                    if (client != null)
                    {
        ...
        public void UpdateUserStatus(RoomUser User, bool cyclegameitems)
        {
            ...
            try
            {
                ...
                if (RocketEmulador.GetUnixTimestamp() > RocketEmulador.GetUnixTimestamp() + User.SignTime)
                {
                    if (User.Statusses.ContainsKey("sign"))
                    {
                        ...
                        case InteractionType.ARROW:
                            {
                                if (User.GoalX == Item.GetX && User.GoalY == Item.GetY)
                                {
                                    ...
                                    if (!RocketEmulador.GetGame().GetRoomManager().TryGetRoom(User.GetClient().GetHabbo().CurrentRoomId, out Room))
                                        return;
                                    ...
                                    else
                                    {
                                        ...
                                        if (TeleRoomId == Room.RoomId)
                                        {
                                            ...
                                            {
                                                if (User.GetClient() != null)
                                                    User.GetClient().SendNotification("Hey, that arrow is poorly!");
                                                return;
                                            }
                                            else
                                            {
