...
namespace Rocket.HabboHotel.Rooms
{
    ...
    {
        private static readonly ILog log = LogManager.GetLogger("Rocket.HabboHotel.Rooms.RoomManager");
        ...
        public RoomManager()
        {
            ...
            Console.WriteLine("       [FUNCIONANDO] => [Rocket Emu] =>  Configurações de quartos carregadas.");
            ...
            using (IQueryAdapter dbClient = RocketEmulador.GetDatabaseManager().GetQueryReactor())
            {
                ...
                foreach (DataRow Row in Data.Rows)
                {
                    ...
                    _roomModels.Add(Modelname, new RoomModel(Convert.ToInt32(Row["door_x"]), Convert.ToInt32(Row["door_y"]), (Double)Row["door_z"], Convert.ToInt32(Row["door_dir"]),
                        Convert.ToString(Row["heightmap"]), Convert.ToString(Row["public_items"]), RocketEmulador.EnumToBool(Row["club_only"].ToString()), Convert.ToString(Row["poolmap"]), Convert.ToInt32(Row["wall_height"])));
                }
            }
        }
        ...
        public List<RoomData> GetOnGoingRoomPromotions(int Mode, int Amount = 6)
        {
        ...
        public RoomData CreateRoom(GameClient Session, string Name, string Description, string Model, int Category, int MaxVisitors, int TradeSettings)
        {
            if (!_roomModels.ContainsKey(Model))
            {
                Session.SendNotification(RocketEmulador.GetGame().GetLanguageLocale().TryGetValue("room_model_missing"));
                return null;
            }

            if (Name.Length < 3)
            {
                Session.SendNotification(RocketEmulador.GetGame().GetLanguageLocale().TryGetValue("room_name_length_short"));
                return null;
            }
