...
{
    class PuxarComando : IChatCommand
    {
        ...
        public void Execute(GameClients.GameClient Session, Rooms.Room Room, string[] Params)
        {
            if (Params.Length == 1)
            {
                Session.SendNotification("Please enter the username of the user you wish to pull.");
                return;
            }

            if (!Room.PullEnabled && !Session.GetHabbo().GetPermissions().HasRight("room_override_custom_config"))
            {
                Session.SendNotification("Oops, it appears that the room owner has disabled the ability to use the pull command in here.");
                return;
            }
            ...
            {
                Session.SendNotification("Please don't pull that user out of the room :(!");
                return;
            }
            ...
            {
                Session.SendNotification("That user is not close enough to you to be pulled, try getting closer!");
                return;
            }
        }
    }
