...
{
    class SeguirJogador : IChatCommand
    {
        ...
        public void Execute(GameClients.GameClient Session, Rooms.Room Room, string[] Params)
        {
            if (Params.Length == 1)
            {
                Session.SendNotification("Please enter the username of the user you wish to follow.");
                return;
            }
            ...
            {
                Session.SendNotification("Hey you, open your eyes! " + TargetClient.GetHabbo().Username + " is in this room!");
                return;
            }

            if (TargetClient.GetHabbo().Username == Session.GetHabbo().Username)
            {
                Session.SendNotification("Sadooooooooo!");
                return;
            }

            if (!TargetClient.GetHabbo().InRoom)
            {
                Session.SendNotification("That user currently isn't in a room!");
                return;
            }

            if (TargetClient.GetHabbo().CurrentRoom.Access != RoomAccess.OPEN && !Session.GetHabbo().GetPermissions().HasRight("mod_tool"))
            {
                Session.SendNotification("Oops, the room that user is either locked, passworded or invisible. You cannot follow!");
                return;
            }
