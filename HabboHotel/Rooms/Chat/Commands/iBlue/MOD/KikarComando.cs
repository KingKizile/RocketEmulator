...
{
    class KikarComando : IChatCommand
    {
        ...
        public void Execute(GameClients.GameClient Session, Rooms.Room Room, string[] Params)
        {
            ...
            Room TargetRoom;
            if (!RocketEmulador.GetGame().GetRoomManager().TryGetRoom(TargetClient.GetHabbo().CurrentRoomId, out TargetRoom))
                return;

            if (Params.Length > 2)
                TargetClient.SendWhisper("A moderator has kicked you from the room for the following reason: " + CommandManager.MergeParams(Params, 2));
            else
                TargetClient.SendWhisper("A moderator has kicked you from the room.");
