...
namespace Rocket.HabboHotel.Rooms.Games.Football
{
    public class Soccer
    {
        private Room _room;
        ...
        public void OnUserWalk(RoomUser User)
        {
            ...
            foreach (Item item in this._balls.Values.ToList())
            {
                ...
                if (differenceX == 0 && differenceY == 0)
                {
                    ...
                    {
                        NewX = User.X;
                        NewY = User.Y + RocketEmulador.Soccer1;

                    }
                    else if (User.RotBody == 6)
                    {
                        NewX = User.X - RocketEmulador.Soccer2;
                        NewY = User.Y;
                    ...
                    {
                        NewX = User.X;
                        NewY = User.Y - RocketEmulador.Soccer3;

                    }
                    else if (User.RotBody == 2)
                    {
                        NewX = User.X + RocketEmulador.Soccer4;
                        NewY = User.Y;

                    }
                    else if (User.RotBody == 1)
                    {
                        NewX = User.X + RocketEmulador.Soccer5;
                        NewY = User.Y - RocketEmulador.Soccer51;

                    }
                    else if (User.RotBody == 7)
                    {
                        NewX = User.X - RocketEmulador.Soccer6;
                        NewY = User.Y - RocketEmulador.Soccer61;

                    }
                    else if (User.RotBody == 3)
                    {
                        NewX = User.X + RocketEmulador.Soccer7;
                        NewY = User.Y + RocketEmulador.Soccer71;

                    }
                    else if (User.RotBody == 5)
                    {
                        NewX = User.X - RocketEmulador.Soccer8;
                        NewY = User.Y + RocketEmulador.Soccer81;
                    }
                    ...
                    if (!this._room.GetRoomItemHandler().CheckPosItem(User.GetClient(), item, NewX, NewY, item.Rotation, false, false))
                    {
                        ...
                        {
                            NewX = User.X;
                            NewY = User.Y + RocketEmulador.Soccer9;
                        }
                        else if (User.RotBody == 2)
                        {
                            NewX = User.X - RocketEmulador.Soccer10;
                            NewY = User.Y;
                        }
                        else if (User.RotBody == 4)
                        {
                            NewX = User.X;
                            NewY = User.Y - RocketEmulador.Soccer11;
                        }
                        else if (User.RotBody == 6)
                        {
                            NewX = User.X + RocketEmulador.Soccer12;
                            NewY = User.Y;
                        }
                        else if (User.RotBody == 5)
                        {
                            NewX = User.X + RocketEmulador.Soccer13;
                            NewY = User.Y - RocketEmulador.Soccer131;
                        }
                        else if (User.RotBody == 3)
                        {
                            NewX = User.X - RocketEmulador.Soccer14;
                            NewY = User.Y - RocketEmulador.Soccer141;
                        }
                        else if (User.RotBody == 7)
                        {
                            NewX = User.X + RocketEmulador.Soccer15;
                            NewY = User.Y + RocketEmulador.Soccer151;
                        }
                        else if (User.RotBody == 1)
                        {
                            NewX = User.X - RocketEmulador.Soccer16;
                            NewY = User.Y + RocketEmulador.Soccer161;
                        }
                    }
                }
                ...
                {
                    NewX = differenceX * -RocketEmulador.Soccer17;
                    NewY = differenceY * -RocketEmulador.Soccer171;
