...
using Rocket.Utilities.Enclosure;
using System.Linq;
...
namespace Rocket.HabboHotel.Rooms.Games.Banzai
{
    public class BattleBanzai
    {
        ...
        public void BanzaiStart()
        {
            ...
            timestarted = RocketEmulador.GetUnixTimestamp();
        ...
        public void BanzaiEnd(bool userTriggered = false)
        {
            ...
            if (winners != TEAM.NONE)
            {
                ...
                foreach (RoomUser User in Winners.ToList())
                {
                    ...
                    {
                        if (RocketEmulador.GetUnixTimestamp() - timestarted > 5)
                        {
                            RocketEmulador.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_BattleBallTilesLocked", User.LockedTilesCount);
                            RocketEmulador.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_BattleBallPlayer", 1);
                        }
                    }
                    if (winners == TEAM.BLUE)
                    {
                        ...
                        {
                            if (RocketEmulador.GetUnixTimestamp() - timestarted > 5)
                                RocketEmulador.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_BattleBallWinner", 1);
