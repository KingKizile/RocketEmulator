...
using Rocket.HabboHotel.Catalog.Pets;
...
namespace Rocket.HabboHotel.Catalog
{
    public class CatalogManager
    {
        private static readonly ILog log = LogManager.GetLogger("Rocket.HabboHotel.Catalog.CatalogManager");
        ...
        public void Init(ItemDataManager ItemDataManager)
        {
                ...
                if (CatalogueItems != null)
                {
                    foreach (DataRow Row in CatalogueItems.Rows)
                    {
                        ...
                        if (!ItemDataManager.GetItem(BaseId, out Data))
                        {
                            ...
                            Console.WriteLine("       [FUNCIONANDO] => [Rocket Emu] =>  Ocorreu um erro ao carregar o item " + ItemId + "por favor cheque a tabela furniture");
                            continue;
                        }
                        ...
                        this._items[PageId].Add(Convert.ToInt32(Row["id"]), new CatalogItem(Convert.ToInt32(Row["id"]), Convert.ToInt32(Row["item_id"]),
                            ...
                            Convert.ToInt32(Row["amount"]), Convert.ToInt32(Row["limited_sells"]), Convert.ToInt32(Row["limited_stack"]), RocketEmulador.EnumToBool(Row["offer_active"].ToString()),
            ...
            Console.ForegroundColor = ConsoleColor.White;
            Console.WriteLine("       [FUNCIONANDO] => [Rocket Emu] =>  Carregando as configurações do catálago.");
        }
