...
using Rocket.HabboHotel.Items.Interactor;
...
namespace Rocket.HabboHotel.Items
{
    ...
    public class Item
    {
        ...
        {
            ItemData Data = null;
            if (RocketEmulador.GetGame().GetItemManager().GetItem(BaseItem, out Data))
            {
                this.Id = Id;
                this.RoomId = RoomId;
                this._room = Room;
                ...
                this.UserID = Userid;
                this.Username = RocketEmulador.GetUsernameById(Userid);
        ...
        public IFurniInteractor Interactor
        {
            get
            {
                bool isWired = this.IsWired;
                IFurniInteractor result;
                if (isWired)
                {
                    result = new InteractorWired();
                }
                else
                {
                    InteractionType interactionType = this.GetBaseItem().InteractionType;
                    if (interactionType <= InteractionType.freezetimer)
                    {
                            ...
                            default:
                                switch (interactionType)
                                {
                                    ...
                                    case InteractionType.banzaicounter:
                                        result = new InteractorBanzaiTimer();
                                        return result;
                                    case InteractionType.freezetimer:
                                        result = new InteractorFreezeTimer();
                                        return result;
                                }
                                break;
                        }
                    ...
                    else
                    {
                        switch (interactionType)
                        {
                            ...
                            case InteractionType.WF_FLOOR_SWITCH_2:
                                result = new InteractorSwitch();
                                return result;
                            default:
                                if (interactionType == InteractionType.MANNEQUIN)
                                {
                                ...
                                switch (interactionType)
                                {
                                    case InteractionType.LOVELOCK:
                                        result = new InteractorLoveLock();
                                        return result;
                                    case InteractionType.CANNON:
                                        result = new InteractorCannon();
                                        return result;
                                    case InteractionType.COUNTER:
                                        result = new InteractorCounter();
                                        return result;
                                }
                                break;
                        }
                    }
                    result = new InteractorGenericSwitch();
                }
                return result;
            }
        }
                                            ...
                                            if (ItemTeleporterFinder.IsTeleLinked(Id, GetRoom()))
                                            {
                                                ...
                                                if (true)
                                                {
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        //RocketEmulador.GetGame().GetRoomManager().AddTeleAction(new TeleUserData(User.GetClient().GetMessageHandler(), User.GetClient().GetHabbo(), RoomId, TeleId));
                                                    }
                                ...
                                if (TargetSquares.Count > 0)
                                {
                                    foreach (Point Square in TargetSquares.ToList())
                                    {
                                        ...
                                        foreach (RoomUser Target in affectedUsers)
                                        {
                                            ...
                                            Target.GetClient().SendMessage(new RoomNotificationComposer("Kikado do quarto !", "Você foi kikado por um canhão!", "room_kick_cannonball", ""));
        ...
        public ItemData GetBaseItem()
        {
            ...
            {
                ItemData I = null;
                if (RocketEmulador.GetGame().GetItemManager().GetItem(this.BaseItem, out I))
                    this._data = I;
            }
