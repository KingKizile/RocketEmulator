...
namespace Rocket.HabboHotel.Items
{
    ...
    {
        private static readonly ILog log = LogManager.GetLogger("Rocket.HabboHotel.Items.ItemDataManager");
        ...
        public void Init()
        {
                ...
                if (ItemData != null)
                {
                    foreach (DataRow Row in ItemData.Rows)
                    {
                        try
                        {
                            ...
                            bool allowStack = RocketEmulador.EnumToBool(Row["can_stack"].ToString());
                            bool allowWalk = RocketEmulador.EnumToBool(Row["is_walkable"].ToString());
                            bool allowSit = RocketEmulador.EnumToBool(Row["can_sit"].ToString());
                            bool allowRecycle = RocketEmulador.EnumToBool(Row["allow_recycle"].ToString());
                            bool allowTrade = RocketEmulador.EnumToBool(Row["allow_trade"].ToString());
                            bool allowMarketplace = Convert.ToInt32(Row["allow_marketplace_sell"]) == 1;
                            bool allowGift = Convert.ToInt32(Row["allow_gift"]) == 1;
                            bool allowInventoryStack = RocketEmulador.EnumToBool(Row["allow_inventory_stack"].ToString());
                            ...
                            bool IsRare = RocketEmulador.EnumToBool(Row["is_rare"].ToString());
                            int ClothingId = Convert.ToInt32(Row["clothing_id"]);
                            bool ExtraRot = RocketEmulador.EnumToBool(Row["extra_rot"].ToString());
            ...
            Console.WriteLine("       [FUNCIONANDO] => [Rocket Emu] =>  Carregando as configurações de itens.");
        }
