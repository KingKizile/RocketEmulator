...
namespace Rocket.HabboHotel.Moderation
{
    ...
    {
        private static ILog log = LogManager.GetLogger("Rocket.HabboHotel.Moderation.ModerationManager");
        ...
        public ModerationManager()
        {
            ...
            Console.WriteLine("       [FUNCIONANDO] => [Rocket Emu] =>  Carregando configurações da moderação.");
        }
        ...
        public void Init()
        {
            ...
            using (IQueryAdapter dbClient = RocketEmulador.GetDatabaseManager().GetQueryReactor())
            {
                ...
                if (GetBans != null)
                {
                    foreach (DataRow dRow in GetBans.Rows)
                    {
                        ...
                        if (Ban != null)
                        {
                            if (expires > RocketEmulador.GetUnixTimestamp())
                            {
            ...
            Console.ForegroundColor = ConsoleColor.White; 
            Console.WriteLine("       [FUNCIONANDO] => [Rocket Emu] =>  Carregando " + (this._userPresets.Count + this._roomPresets.Count) + " ajustes.");
            Console.WriteLine("       [FUNCIONANDO] => [Rocket Emu] =>  Carregando " + this._userActionPresetCategories.Count + " categorias.");
            Console.WriteLine("       [FUNCIONANDO] => [Rocket Emu] =>  Carregando " + this._userActionPresetMessages.Count + " mensagens da moderação.");
            Console.WriteLine("       [FUNCIONANDO] => [Rocket Emu] =>  Carregando " + this._bans.Count + " usuários banidos.");
        }
        ...
        public void ReCacheBans()
        {
            ...
            Console.ForegroundColor = ConsoleColor.White;
            Console.WriteLine("       [FUNCIONANDO] => [Rocket Emu] =>  Carregando" + this._bans.Count + " usuários e máquinas banidas.");
        ...
        public void BanUser(string Mod, ModerationBanType Type, string BanValue, string Reason, double ExpireTimestamp)
        {
            ...
            using (IQueryAdapter dbClient = RocketEmulador.GetDatabaseManager().GetQueryReactor())
            {
                dbClient.SetQuery("REPLACE INTO `bans` (`bantype`, `value`, `reason`, `expire`, `added_by`,`added_date`) VALUES ('" + BanType + "', '" + BanValue + "', @reason, " + ExpireTimestamp + ", '" + Mod + "', '" + RocketEmulador.GetUnixTimestamp() + "');");
        ...
        public bool MachineBanCheck(string MachineId)
        {
            ...
            if (RocketEmulador.GetGame().GetModerationManager().IsBanned(MachineId, out MachineBanRecord))
            {
                DataRow BanRow = null;
                using (IQueryAdapter dbClient = RocketEmulador.GetDatabaseManager().GetQueryReactor())
                {
                    ...
                    if (BanRow == null)
                    {
                        RocketEmulador.GetGame().GetModerationManager().RemoveBan(MachineId);
                        return false;
                    }
                }
            }
        ...
        public bool UsernameBanCheck(string Username)
        {
            ...
            if (RocketEmulador.GetGame().GetModerationManager().IsBanned(Username, out UsernameBanRecord))
            {
                DataRow BanRow = null;
                using (IQueryAdapter dbClient = RocketEmulador.GetDatabaseManager().GetQueryReactor())
                {
                    ...
                    if (BanRow == null)
                    {
                        RocketEmulador.GetGame().GetModerationManager().RemoveBan(Username);
                        return false;
                    }
                }
            }
