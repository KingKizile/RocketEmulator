...
namespace Rocket.HabboHotel.Support
{
    ...
    public class ModerationTool
    {
        ...
        public void SendNewTicket(GameClient Session, int Category, int ReportedUser, String Message, List<string> Messages, int Type = 1, int ExtraId = 0)
        {
            int TicketId = 0;
            ...
            if (Session.GetHabbo().CurrentRoomId <= 0)
            {
                ...
                {
                    dbClient.SetQuery("INSERT INTO moderation_tickets (score,type,status,sender_id,reported_id,moderator_id,message,room_id,room_name,timestamp) VALUES (1,'" + Category + "', 'open', '" + Session.GetHabbo().Id + "', '" + ReportedUser + "', '0', @message, '0', '', '" + RocketEmulador.GetUnixTimestamp() + "')");
                ...
                Ticket = new SupportTicket(TicketId, 1, 7, Category, Session.GetHabbo().Id, ReportedUser, Message, 0, "", RocketEmulador.GetUnixTimestamp(), Messages);
            ...
            RoomData Data = RocketEmulador.GetGame().GetRoomManager().GenerateRoomData(Session.GetHabbo().CurrentRoomId);

            using (IQueryAdapter dbClient = RocketEmulador.GetDatabaseManager().GetQueryReactor())
            {
                dbClient.SetQuery("INSERT INTO moderation_tickets (score,type,status,sender_id,reported_id,moderator_id,message,room_id,room_name,timestamp) VALUES (1, '" + Category + "', 'open', '" + Session.GetHabbo().Id + "', '" + ReportedUser + "', '0', @message, '" + Data.Id + "', @name,'" + RocketEmulador.GetUnixTimestamp() + "')");
                dbClient.AddParameter("message", Message);
            ...
            Ticket = new SupportTicket(TicketId, 1, 7, Category, Session.GetHabbo().Id, ReportedUser, Message, Data.Id, Data.Name, RocketEmulador.GetUnixTimestamp(), Messages);
            Tickets.Add(Ticket);
            SendTicketToModerators(Ticket);
        }
        ...
        public void CloseTicket(GameClient Session, int TicketId, int Result)
        {
            ...
            GameClient Client = RocketEmulador.GetGame().GetClientManager().GetClientByUserID(Ticket.SenderId);
        ...
        {
            RocketEmulador.GetGame().GetClientManager().SendMessage(new ModeratorSupportTicketComposer(Ticket), "mod_tool");
        }
