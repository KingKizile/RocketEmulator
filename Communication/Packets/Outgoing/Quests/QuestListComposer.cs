...
namespace Rocket.Communication.Packets.Outgoing.Quests
{
    public class QuestListComposer : ServerPacket
    {
        ...
        private void SerializeQuest(ServerPacket Message, GameClient Session, Quest Quest, string Category)
        {
            ...
            int AmountInCat = RocketEmulador.GetGame().GetQuestManager().GetAmountOfQuestsInCategory(Category);
            ...
            Message.WriteInteger(Quest == null ? 2 : Quest.RewardType);// Reward type (1 = Snowflakes, 2 = Love hearts, 3 = Pixels, 4 = Seashells, everything else is pixels
            Message.WriteInteger(Quest == null ? 0 : Quest.Id); // Quest id
            Message.WriteBoolean(Quest == null ? true : Session.GetHabbo().GetStats().QuestID == Quest.Id);  // Quest started
            ...
            Message.WriteInteger(Quest == null ? 111 : Quest.GoalData); // Target progress
            Message.WriteInteger(Quest == null ? 111110 : Quest.TimeUnlock); // "Next quest available countdown" in seconds
            Message.WriteString("");
            Message.WriteString("");
