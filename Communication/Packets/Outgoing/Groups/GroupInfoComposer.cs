...
namespace Rocket.Communication.Packets.Outgoing.Groups
{
    class GroupInfoComposer : ServerPacket
    {
            ...
            : base(ServerPacketHeader.GroupInfoMessageComposer)
        {
            ...
            base.WriteString((RocketEmulador.GetGame().GetRoomManager().GenerateRoomData(Group.RoomId) == null) ? "No room found.." : RocketEmulador.GetGame().GetRoomManager().GenerateRoomData(Group.RoomId).Name);    // room name
            ...
            base.WriteString(RocketEmulador.GetUsernameById(Group.CreatorId));
