...
namespace Rocket.Communication.Packets.Outgoing.Moderation
{
    class ModeratorUserChatlogComposer : ServerPacket
    {
            ...
            : base(ServerPacketHeader.ModeratorUserChatlogMessageComposer)
        {
            ...
            base.WriteString(RocketEmulador.GetGame().GetClientManager().GetNameById(UserId));
            using (IQueryAdapter dbClient = RocketEmulador.GetDatabaseManager().GetQueryReactor())
            {
                ...
                if (Visits != null)
                {
                    ...
                    foreach (DataRow Visit in Visits.Rows)
                    {
                        ...
                        Room Room = RocketEmulador.GetGame().GetRoomManager().LoadRoom(Convert.ToInt32(Visit["room_id"]));
                        ...
                        {
                            Visit["exit_timestamp"] = RocketEmulador.GetUnixTimestamp();
                        }
                        ...
                        dbClient.SetQuery("SELECT user_id,DATE_FORMAT(from_unixtime(timestamp),'%k:%i'),message FROM `chatlogs` WHERE room_id = " + Convert.ToInt32(Visit["room_id"]) + " AND user_id = '" + UserId + "' ORDER BY id DESC LIMIT 150");
                        ...
                        if (Chatlogs != null)
                        {
                            ...
                            {
                                UserCache Habbo = RocketEmulador.GetGame().GetCacheManager().GenerateUser(Convert.ToInt32(Log["user_id"]));

                                if (Habbo == null)
                                    continue;
                                //base.WriteInteger(((int)RocketEmulador.GetUnixTimestamp() - Convert.ToInt32(Log["timestamp"])) * 1000);
                                //int Time = ((int)RocketEmulador.GetUnixTimestamp() - Convert.ToInt32(Log["timestamp"]) * 1000);
                                //int Time = (int)RocketEmulador.GetUnixTimestamp();
