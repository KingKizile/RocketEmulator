                ...
                case 2:
                    {
                        ...
                        for (int i = 0; i <= SpeechData.Length - 1; i++)
                        {
                            using (IQueryAdapter dbClient = RocketEmulador.GetDatabaseManager().GetQueryReactor())
                            {
                                ...
                                dbClient.AddParameter("MixChat", RocketEmulador.BoolToEnum(Convert.ToBoolean(MixChat)));
                                dbClient.RunQuery();
                            }
                        }
                        #endregion
                ...
                case 5:
                    {
                        ...
                        {
                            Session.SendNotification("Come on, atleast give the bot a name!");
                            return;
                        }
                        else if (DataString.Length >= 16)
                        {
                            Session.SendNotification("Come on, the bot doesn't need a name that long!");
                            return;
                        }
                        ...
                        {
                            Session.SendNotification("No HTML, please :<");
                            return;
                        }
