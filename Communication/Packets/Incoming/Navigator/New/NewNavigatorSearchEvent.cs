    ...
    class NewNavigatorSearchEvent : IPacketEvent
    {
        public void Parse(HabboHotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            if (!string.IsNullOrEmpty(Unknown))
            {
                ...
                if (RocketEmulador.GetGame().GetNavigator().TryGetSearchResultList(0, out Null))
                {
                    Test.Add(Null);
            ...
            {
                //Fetch the categorys.
                ICollection<SearchResultList> Test = RocketEmulador.GetGame().GetNavigator().GetCategorysForSearch(Category);
                if (Test.Count == 0)
                {
                    ICollection<SearchResultList> SecondTest = RocketEmulador.GetGame().GetNavigator().GetResultByIdentifier(Category);
