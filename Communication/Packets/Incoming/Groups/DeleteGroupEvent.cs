...
namespace Rocket.Communication.Packets.Incoming.Groups
{
    class DeleteGroupEvent : IPacketEvent
    {
        public void Parse(HabboHotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            if (Group.MemberCount >= RocketGame.GroupMemberDeletionLimit && !Session.GetHabbo().GetPermissions().HasRight("group_delete_limit_override"))
            {
                Session.SendNotification("Oops, your group exceeds the maximum amount of members (" + RocketGame.GroupMemberDeletionLimit + ") a group can exceed before being eligible for deletion. Seek assistance from a staff member.");
                return;
            }

            Room Room = RocketEmulador.GetGame().GetRoomManager().LoadRoom(Group.RoomId);
            ...
            RocketEmulador.GetGame().GetGroupManager().DeleteGroup(Group.Id);
