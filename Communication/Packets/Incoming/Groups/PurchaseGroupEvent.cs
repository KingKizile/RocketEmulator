...
namespace Rocket.Communication.Packets.Incoming.Groups
{
    class PurchaseGroupEvent : IPacketEvent
    {
        ...
        {
            if (Session.GetHabbo().Credits < RocketGame.GroupPurchaseAmount)
            {
                Session.SendMessage(new BroadcastMessageAlertComposer("A group costs " + RocketGame.GroupPurchaseAmount + " credits! You only have " + Session.GetHabbo().Credits + "!"));
                return;
            }
            else
            {
                Session.GetHabbo().Credits -= RocketGame.GroupPurchaseAmount;
            ...
            RoomData Room = RocketEmulador.GetGame().GetRoomManager().GenerateRoomData(RoomId);
            ...
            Symbol1 = RocketEmulador.GetGame().GetGroupManager().CheckActiveSymbol(Symbol1);
            Symbol2 = RocketEmulador.GetGame().GetGroupManager().CheckActiveSymbol(Symbol2);
            Symbol3 = RocketEmulador.GetGame().GetGroupManager().CheckActiveSymbol(Symbol3);
            Symbol4 = RocketEmulador.GetGame().GetGroupManager().CheckActiveSymbol(Symbol4);

            string Badge = Base + Symbol1 + Symbol2 + Symbol3 + Symbol4;

            Group Group = null;
            if (!RocketEmulador.GetGame().GetGroupManager().TryCreateGroup(Session.GetHabbo(), Name, Description, RoomId, Badge, Colour1, Colour2, out Group))
            {
