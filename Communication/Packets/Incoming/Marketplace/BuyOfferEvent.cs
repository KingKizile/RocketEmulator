...
using Rocket.HabboHotel.Catalog.Marketplace;
...
namespace Rocket.Communication.Packets.Incoming.Marketplace
{
    class BuyOfferEvent : IPacketEvent
    {
        public void Parse(HabboHotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            if (RocketEmulador.GetGame().GetCatalog().GetMarketplace().FormatTimestamp() > (Convert.ToDouble(Row["timestamp"])))
            {
            ...
            ItemData Item = null;
            if (!RocketEmulador.GetGame().GetItemManager().GetItem(Convert.ToInt32(Row["item_id"]), out Item))
            {
            ...
            else
            {
                ...
                if (GiveItem != null)
                {
                    ...
                    Session.SendMessage(new Rocket.Communication.Packets.Outgoing.Catalog.PurchaseOKComposer());
                ...
                using (IQueryAdapter dbClient = RocketEmulador.GetDatabaseManager().GetQueryReactor())
                {
                    ...
                    if (RocketEmulador.GetGame().GetCatalog().GetMarketplace().MarketAverages.ContainsKey(Item.SpriteId) && RocketEmulador.GetGame().GetCatalog().GetMarketplace().MarketCounts.ContainsKey(Item.SpriteId))
                    {
                        int num3 = RocketEmulador.GetGame().GetCatalog().GetMarketplace().MarketCounts[Item.SpriteId];
                        int num4 = (RocketEmulador.GetGame().GetCatalog().GetMarketplace().MarketAverages[Item.SpriteId] += Convert.ToInt32(Row["total_price"]));

                        RocketEmulador.GetGame().GetCatalog().GetMarketplace().MarketAverages.Remove(Item.SpriteId);
                        RocketEmulador.GetGame().GetCatalog().GetMarketplace().MarketAverages.Add(Item.SpriteId, num4);
                        RocketEmulador.GetGame().GetCatalog().GetMarketplace().MarketCounts.Remove(Item.SpriteId);
                        RocketEmulador.GetGame().GetCatalog().GetMarketplace().MarketCounts.Add(Item.SpriteId, num3 + 1);
                    }
                    else
                    {
                        if (!RocketEmulador.GetGame().GetCatalog().GetMarketplace().MarketAverages.ContainsKey(Item.SpriteId))
                            RocketEmulador.GetGame().GetCatalog().GetMarketplace().MarketAverages.Add(Item.SpriteId, Convert.ToInt32(Row["total_price"]));

                        if (!RocketEmulador.GetGame().GetCatalog().GetMarketplace().MarketCounts.ContainsKey(Item.SpriteId))
                            RocketEmulador.GetGame().GetCatalog().GetMarketplace().MarketCounts.Add(Item.SpriteId, 1);
                    }
                }
            }
        ...
        private void ReloadOffers(GameClient Session)
        {
            ...
            string str = "";
            builder.Append("WHERE state = '1' AND timestamp >= " + RocketEmulador.GetGame().GetCatalog().GetMarketplace().FormatTimestampString());
            if (MinCost >= 0)
            {
            ...
            RocketEmulador.GetGame().GetCatalog().GetMarketplace().MarketItems.Clear();
            RocketEmulador.GetGame().GetCatalog().GetMarketplace().MarketItemKeys.Clear();
            if (table != null)
            {
                foreach (DataRow row in table.Rows)
                {
                    if (!RocketEmulador.GetGame().GetCatalog().GetMarketplace().MarketItemKeys.Contains(Convert.ToInt32(row["offer_id"])))
                    {
                        ...
                        RocketEmulador.GetGame().GetCatalog().GetMarketplace().MarketItemKeys.Add(Convert.ToInt32(row["offer_id"]));
                        RocketEmulador.GetGame().GetCatalog().GetMarketplace().MarketItems.Add(item);
                    }
                }
            }
            ...
            foreach (MarketOffer item in RocketEmulador.GetGame().GetCatalog().GetMarketplace().MarketItems)
            {
